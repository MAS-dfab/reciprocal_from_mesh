...
class ReciprocalFrame(Graph):
    ...
    def from_mesh(cls, mesh):
        ...
        def get_beam(u, v):
            ...
            p1 = centroids[f1] if f1 is not None else Point(*midpoint_line((mesh.vertex_coordinates(u), mesh.vertex_coordinates(v))))
            p2 = centroids[f2] if f2 is not None else Point(*midpoint_line((mesh.vertex_coordinates(u), mesh.vertex_coordinates(v))))
        ...
        for fkey in mesh.faces():
            ...
            for u, v in mesh.face_halfedges(fkey):
                ...
                param = 0 if beam_line.start.distance_to_point(face_centroid) < 1e-6 else 1
    ...
    def _build_connection_arrays(self, keys):
        ...
        return (np.array(i_idx, dtype=np.int32),
                np.array(end_i, dtype=np.int32),
                np.array(j_idx, dtype=np.int32),
                np.array(end_j, dtype=np.int32),
                np.array(xi_arr))
    
    def _build_sparsity(self, n_beams, n_conn, n_res, i_idx, j_idx, wt, we, wf, ws, has_srf):
    ...
    def solve(self, weights=(1.0, 0.0, 0.1, 0.0), eccentricity=0.00, srf=None, use_sparse=True):
        """
        ...
        n_res = 0
        if wt > 0: n_res += n_conn * 3
        if we > 0: n_res += n_conn
        if wf > 0: n_res += n_beams * 6
        if ws > 0 and srf is not None: n_res += n_beams * 2
        
        srf_val = float(srf) if srf is not None and isinstance(srf, (int, float)) else 0.0
        ...
        if use_sparse and n_beams > 50:
            sparsity = self._build_sparsity(n_beams, n_conn, n_res, i_idx, j_idx, wt, we, wf, ws, has_srf)
            result = least_squares(residuals, x0.flatten(), method='trf', jac_sparsity=sparsity,
                                   ftol=1e-8, xtol=1e-8, max_nfev=5000)
        else:
            result = least_squares(residuals, x0.flatten(), method='lm', ftol=1e-8, xtol=1e-8, max_nfev=5000)
